...
class Builder:
    ...
    def build(self, version: str, build_number: str) -> Build:
        ...
        unpacked = out / "uDeck.app"
        if unpacked.exists():
            raise LabError(step, f"{MAKE_APP} left {unpacked} unpacked on this Mac")
    ...
    def _call(self, args: list[str], step: str, log: Path) -> subprocess.CompletedProcess[str]:
        try:
            done = self._run(
                args,
                capture_output=True,
            ...
            raise LabError(step, f"'{MAKE_APP}' did not finish in {self.seconds:.0f}s") from None
        except OSError as error:
            raise LabError(step, f"could not run '{MAKE_APP}': {error}") from None
        try:
            log.write_text(done.stdout + done.stderr, encoding="utf-8", errors="backslashreplace")
        except OSError as error:
        ...
        if done.returncode != 0:
            ...
            raise LabError(step, f"'{MAKE_APP}' exited {done.returncode}: {said}")
        return done
